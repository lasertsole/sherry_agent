...
- [x] 定义 Rust commands: `read_text_file`、`write_text_file`、`get_app_data_dir`、`show_notification`
- [x] 更新 `tauri.conf.json`: productName → "EMA AI Agent", identifier → 实际 bundle ID, 添加系统托盘配置
- [x] 添加 Tauri 插件: `tauri-plugin-notification`, `tauri-plugin-shell`（按需）
- [x] 实现系统托盘菜单（显示/隐藏、退出）
- [x] 实现全局快捷键（如 Alt+Space 切换窗口）
- [x] 错误处理：通过 Tauri `Result<T, E>` 向前端返回结构化错误
...
- [x] 消息气泡组件，支持 Markdown 渲染
- [x] 区分用户消息和 AI 消息（左右对齐 + 样式差异）
- [x] 消息元信息：时间戳、模型名、token 数
- [x] 新消息自动滚动到底部
- [x] AI 响应时的加载/流式指示器
- [x] 支持多模态消息：文本 + 图片
- [x] 复制消息文本按钮
...
- [x] 多行文本输入：Enter 发送 / Shift+Enter 换行
- [x] 流式响应中发送按钮置为禁用状态
- [x] 流式响应中的停止生成按钮
- [x] 图片上传（multipart/form-data 通过 `requestApi.ts`）
- [ ] 文件上传附件
- [x] 知识库开关

#### 1.5 流式响应 & SSE 集成

- [x] 消费 Python 后端 `/chat/stream` 端点的 SSE / chunked JSON 响应
- [x] 逐块增量更新消息气泡内容
- [x] 处理流取消（abort fetch + 通知后端）
- [x] 处理网络错误重连
...
- [x] i18n JSON 文件: zh.json、en.json、ja.json
- [x] 翻译所有 UI 文案（侧边栏、输入框、按钮、提示、空状态）
- [x] 确保多语言切换无缝工作

#### 1.8 Tauri 桌面集成

- [x] 系统托盘图标 + 右键菜单（显示、隐藏、退出）
- [x] 全局快捷键 Alt+Space 切换窗口显示/隐藏
- [x] 单实例锁（防止多开窗口）
- [x] 窗口状态持久化（位置、大小、最大化状态）

#### 1.9 打磨与测试

- [x] 错误处理 UI: API 错误 toast 通知、离线提示
- [x] 加载骨架屏 / spinner 组件
- [ ] 响应式布局：移动端、平板、桌面端测试
- [x] 暗色模式一致性（所有组件的 tailwind `dark:` 类）
- [ ] 无障碍：键盘导航、焦点管理
