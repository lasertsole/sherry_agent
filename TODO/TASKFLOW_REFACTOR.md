...
> 3. **Knowledge 工具级隔离已补齐（计划归属）**：`knowledge` 工具本身强制归属校验——`ownership.is_plan_associated()` 取三来源并集（会话 `plan_ref` 状态键；本会话 todos 的 `plan_ref`，SQL 按 `session_id` 过滤；`.omo/boulder.json` 中 `plan_name` 匹配且 `session_ids` 含本会话的 work）。`write` / `read` 对非关联计划返回可诊断错误，`list` 仅列本会话关联计划；多会话协同（boulder `session_ids` 含多个会话）仍可读写。工具新增 `session_id: Annotated[str, InjectedState("session_id")]`；nudge agent 的 `StateSchema` 已携带 `session_id`（构造时 seed），写入路径不受影响。未加会话列；`build_knowledge_block(session_id)` 仍按会话解析 `plan_ref` 供提示块使用；子 agent 边界不变（`knowledge` 为 `main_only`）。测试落在 `tests/agent/tools/todolist/knowledge/test_knowledge_ownership.py`（三来源命中/未命中、boulder 缺失/损坏、跨会话拒绝、list 收敛、多会话协同、nudge `session_id` 实证）。
