...
> 3. **Knowledge 隔离按计划身份落地**：工具层强制归属——`ownership.association_plan_refs()` 取三来源（会话 `plan_ref` 状态键；本会话 todos 的 `plan_ref`，SQL 按 `session_id` 过滤；`.omo/boulder.json` 中 `plan_name` 匹配且 `session_ids` 含本会话的 work）；`identity.resolve_plan_identity()` 将计划名解析到规范化计划路径并派生存储键 `sha1(相对仓库根路径)[:12]`，目录 `workspace/knowledge/plans/<plan_key>/` 内附 `meta.json`（记录可读 `plan_name` / `plan_ref`）。同名不同计划文件物理隔离；通过 boulder `session_ids` 协同同一计划文件的会话共享同一 `<plan_key>`；本会话 `plan_ref` 在多个同名路径间优先，仍歧义则拒绝（不静默落到别处）；无计划文件的会话写入兜底身份 `session-<sha1(session_id)[:8]>`（全 id 哈希，消除前 8 字符相同 id 的兜底互撞）。`write` / `read` 对非关联或歧义计划返回可诊断错误，`list` 仅列本会话关联计划（可读名 + key）；旧名称为键目录只读回退，写入总是落新 key；`clear_session` 删除本会话私有身份目录并保留 boulder 共享计划的知识（legacy 目录不删）。nudge 写入路径同源解析；子 agent 边界不变（`knowledge` 为 `main_only`）。测试：`tests/agent/tools/todolist/knowledge/test_knowledge_identity.py`（同名隔离 / 协同共享 / legacy 回退 / 兜底哈希 / clear_session）及 store / tool / ownership 既有用例按新契约更新。
