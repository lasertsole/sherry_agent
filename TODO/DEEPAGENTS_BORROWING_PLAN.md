...
> 优先级：P1(增强体验) → P2(长期优化)
>
> **未执行项清单**：P1-3 模型感知摘要默认值、P1-8 威胁模型文档、P2-1 ripgrep 双重超时看门狗（已完成项见 git 历史）。
>
...
>
> **已落地**：P1-6 中间件脚手架保护（组装时校验 + 漂移守卫）。落点与提案的差异：必需集合**在实施时逐项对齐当前真实链**——主链 17 项中取 12 项安全/可靠性项（不含 `TodoContinuationEnforcer` / `system_prompt_injection` / `MultimodalProcessor` / `SubagentCompletionDrainMiddleware` / `TaskIntentMiddleware`），子链 7 项全取；两个组装入口在 `create_agent(...)` 前用**局部变量**装配再校验；子链中间件列表额外提取为 `spawn/core.py::_build_child_middlewares()`，使漂移守卫能直接校验真实链（而非手工复刻）。
...
2. [P1-8：威胁模型文档](#p1-8威胁模型文档)
3. [P2-1：ripgrep 双重超时看门狗](#p2-1ripgrep-双重超时看门狗)
4. [实施优先级与依赖关系总览](#实施优先级与依赖关系总览)
...
- **独立实施**：P1-3 模型感知摘要默认值、P1-8 威胁模型文档
- **已落地**：P1-6 中间件脚手架保护（组装时校验 + 漂移守卫测试；主/子链必需集合对齐当前真实链，子链额外提取 `spawn/core.py::_build_child_middlewares()` 以便直接校验真实链）
