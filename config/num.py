...
MAX_OVERFLOW_RETRIES = 3  # T4/T5 overflow-handling retry cap
MAX_COMPRESS_ATTEMPTS_PER_TURN = 3  # Max compression attempts per turn
COMPACTION_COOLDOWN_ROUNDS = 3  # Cooldown model-call rounds after compaction
PRUNE_TTL_SECONDS = 300  # Tool-result TTL (5 minutes)
TRUNCATE_BUDGET_RATIO = 0.6  # Tool-result truncation budget as a share of usable_budget
MIN_TOOL_RESULT_TOKENS_TO_TRUNCATE = 200  # Below this token count truncation isn't worth it
TRUNCATABLE_RECENT_SKIP = 6  # Skip the 6 most recent messages (~2 turns)
TTL_REGISTRY_MAX_ENTRIES = 512  # TTL registry capacity cap, prevents unbounded growth
