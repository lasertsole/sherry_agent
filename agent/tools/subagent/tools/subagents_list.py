...
class SubagentsListTool(BaseTool):
    ...
    description: str = "List active and recent subagent runs for the current session."
    ...
    async def _arun(self) -> str:
        ...
        lines = [
            f"Subagents: total={info['total']}, active={info['active_count']}, recent={info['recent_count']}"
        ]
        ...
        if info["active"]:
            ...
            for a in info["active"]:
                lines.append(
                    f"  - [{a['run_id'][:8]}] {a.get('label', a['task'])} (depth={a['depth']}, role={a['role']})"
                )
        ...
        if info["recent"]:
            ...
            for r in info["recent"]:
                lines.append(
                    f"  - [{r['run_id'][:8]}] {r.get('label', r['task'])} status={r['status']}"
                )
