...
class CommunityComparison(TypedDict):
    """A/B comparison result for undirected vs directed community detection"""
    node_count: int
    edge_count: int
    undirected: dict
    directed: dict
...
def compare_community_detection(db: Connection) -> CommunityComparison:
    """
    A/B comparison: run community detection with undirected vs directed graph on the same data.
    Does NOT write results to DB. Returns comparison metrics for analysis.

    Returns:
        Comparison result with both versions' community info and label differences
    """
    import time
    ...
    if not node_ids:
        return {
            ...
            "undirected": {}, "directed": {}, "labels_diff": {},
        }
    ...
    # ── Undirected ──
    g_undir = ig.Graph(len(node_ids), edges, directed=False)
    g_undir.simplify(multiple=True, loops=True)
    res_undir = _compute_safe_resolution(g_undir, directed=False)

    t0 = time.time()
    part_undir = leidenalg.find_partition(
        g_undir, leidenalg.CPMVertexPartition,
        resolution_parameter=res_undir, n_iterations=-1,
    )
    undir_ms = int((time.time() - t0) * 1000)

    undir_labels = {node_ids[i]: part_undir.membership[i] for i in range(len(node_ids))}

    undir_communities: dict[int, list[str]] = {}
    for nid, cid in undir_labels.items():
        undir_communities.setdefault(cid, []).append(nid)

    # ── Directed ──
    g_dir = ig.Graph(len(node_ids), edges, directed=True)
    g_dir.simplify(multiple=True, loops=True)
    res_dir = _compute_safe_resolution(g_dir, directed=True)

    t0 = time.time()
    part_dir = leidenalg.find_partition(
        g_dir, leidenalg.CPMVertexPartition,
        resolution_parameter=res_dir, n_iterations=-1,
    )
    dir_ms = int((time.time() - t0) * 1000)

    dir_labels = {node_ids[i]: part_dir.membership[i] for i in range(len(node_ids))}

    dir_communities: dict[int, list[str]] = {}
    for nid, cid in dir_labels.items():
        dir_communities.setdefault(cid, []).append(nid)

    # ── Compare: how many nodes changed community ──
    # Normalize labels to 0-based sequential for comparison
    undir_norm = _normalize_labels(undir_labels)
    dir_norm = _normalize_labels(dir_labels)

    changed = 0
    same = 0
    for nid in node_ids:
        if undir_norm.get(nid) == dir_norm.get(nid):
            same += 1
        else:
            changed += 1

    # ── Per-community detail ──
    def _community_detail(communities: dict, labels: dict) -> list[dict]:
        cursor2 = db.cursor()
        result = []
        for cid, members in sorted(communities.items(), key=lambda x: -len(x[1])):
            ...
            cursor2.execute(
                f"SELECT name, type FROM gm_nodes WHERE id IN ({placeholders})",
                (*member_ids,),
            )
            nodes_info = [f"{r['type']}:{r['name']}" for r in cursor2.fetchall()]
            result.append({
                "id": cid,
                "size": len(members),
                "sample_nodes": nodes_info[:5],
            })
        return result

    undir_detail = _community_detail(undir_communities, undir_labels)
    dir_detail = _community_detail(dir_communities, dir_labels)

    return {
        "node_count": len(node_ids),
        "edge_count": len(edges),
        "undirected": {
            "communities": len(undir_communities),
            "resolution": round(res_undir, 4),
            "density": round(g_undir.density(), 4),
            "quality": round(part_undir.quality(), 4),
            "duration_ms": undir_ms,
            "detail": undir_detail,
        },
        "directed": {
            "communities": len(dir_communities),
            "resolution": round(res_dir, 4),
            "density": round(g_dir.density(), 4),
            "quality": round(part_dir.quality(), 4),
            "duration_ms": dir_ms,
            "detail": dir_detail,
        },
        "labels_diff": {
            "same": same,
            "changed": changed,
            "change_rate": round(changed / len(node_ids), 4) if node_ids else 0,
        },
    }
