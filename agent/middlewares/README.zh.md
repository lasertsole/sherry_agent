...
| 关注点 | 解决方案 |
|--------|----------|
| 计数器位置 | 存储在 `state_register_mem["iteration_budget"]["count"]`，跨工具调用持久化 |
...
输入：AgentState
        │
        ▼
1. 自增 state_register_mem["iteration_budget"]["count"] += 1
```
...
| 特性 | Summarization | ToolCallNormalize | ToolGuardrails | ToolTimeout | IterationBudget | MultimodalProcessor | ContextEngineHook |
|------|---------------|-------------------|----------------|-------------|-----------------|---------------------|-------------------|
| **基类** | `SummarizationMiddleware` | `AgentMiddleware` | `AgentMiddleware` | `AgentMiddleware` | `AgentMiddleware` | `AgentMiddleware` | `ContextEngineHook` |
| **触发时机** | 模型调用前 | Agent 前 | 每次工具调用（`awrap_tool_call`） | 每次工具调用（`awrap_tool_call`） | 工具返回后（`awrap_after_tool`） | Agent 前后 | Agent 前后 |
| **核心操作** | 压缩 + 偏好提取 | 消息配对修复 | 三级防护（warn/block/halt） | 超时控制 + 错误返回 | 迭代计数 + 强制结束 | 图片解码 + 临时文件清理 | 上下文增强 + 持久化 |
| **阻塞性** | 同步阻塞 | 同步 | 同步 | 异步阻塞 | 同步 | 同步 | 异步非阻塞（after 部分） |
| **依赖** | MesMemory、`memory_store` | `sanitize_tool_use_result_pairing` | 无 | `asyncio.wait_for` | 无 | PIL (Pillow) | Context Engine |
| **频率** | 仅上下文过长时 | 每轮 Agent 推理 | 每次工具调用 | 每次工具调用 | 每次工具返回 | 每轮 Agent 推理 | 每轮 Agent 推理 |
| **消息变更** | 克隆 + 修改副本 | 替换整个消息列表 | 注入 SystemMessage / 返回 error ToolMessage | 返回 error ToolMessage | 注入 SystemMessage | 替换图片内容块 | 原地修改 |
    ...
    rect rgb(240, 255, 240)
        ...
        loop 工具调用循环
            ...
            alt 达到 halt 阈值
                ...
                Guard->>Guard: 注入警告 SystemMessage
                Guard->>TO: 转发 handler
...
| 阶段 | Summarization | ToolCallNormalize | ToolGuardrails | ToolTimeout | IterationBudget | MultimodalProcessor | ContextEngineHook |
|------|---------------|-------------------|----------------|-------------|-----------------|---------------------|-------------------|
| **Before Agent** | — | `sanitize_tool_use_result_pairing()` → 修复工具配对，移除孤立消息 | 重置调用计数器 | — | 注入"立即回答"提示（预算超限时） | 查找 image_url → base64 解码 → 写入临时文件 → 替换内容块 | 剥离系统消息 → 提取查询 → 构造增强 → 前置拼接 |
| **Before Model** | 克隆 → 剥离系统消息 → 压缩 → 插回 → nudge | — | 注入警告 SystemMessage（检测到循环时） | — | — | — | — |
| **LLM 推理（每次工具调用）** | — | — | 三级判定（warn/block/halt） | `asyncio.wait_for` → 超时返回 error | — | — | — |
| **After Tool** | — | — | 注册工具执行结果 | 检查耗时 vs 超时阈值 | 自增迭代计数器 | — | — |
| **After Agent** | 存储摘要结果 | 更新 last_names 跟踪 | — | — | — | 扫描 mutil_temp → 删除超 7 天文件 | 提取最后一轮 → 清理工具配对 → 还原输入 → 异步持久化 |
...
`ToolGuardrails` 和 `ToolTimeout` 都使用了相同的设计模式 — `awrap_tool_call` 包装器。LangGraph Runtime 会在每次工具调用时调用 `awrap_tool_call`，传入原始请求和一个 `handler` 函数（代表下一个中间件或实际的工具执行器）：
...
ToolGuardrails、ToolTimeout 和 IterationBudget 构成了工具调用的三层安全防护：
...
由 `ToolGuardrails` 管理，每轮在 `abefore_agent` 中重置。
...
| 配置项 | ContextEngineHook | Summarization | ToolGuardrails | ToolCallNormalize | ToolTimeout | MultimodalProcessor |
|--------|-------------------|---------------|---------------------|-------------------|-------------|---------------------|
...
# abefore_agent 执行顺序：Summarization → ToolCallNormalize → ToolGuardrails → ContextEngineHook
# awrap_tool_call 执行顺序：ToolGuardrails → ToolTimeout → 实际工具
# aafter_agent 执行顺序：ContextEngineHook → MultimodalProcessor
runtime = Runtime(
    agent=my_agent,
    middlewares=[summarizer, normalizer, guardrails, timeout_mw, budget, processor, context_hook]
)
```
...
### Q8: 什么时候应该使用 `ToolGuardrails` vs `ToolTimeout`？

它们解决不同的问题：
- **ToolGuardrails** 防护的是*调用序列失控* — 模型在单轮内反复调用同一个工具（例如连续调用 `web_search` 50 次翻页）。它限制的是调用*次数*，且支持三级自动升级（warn → block → halt）。
