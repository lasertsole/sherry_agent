...
- **Programmatic gate (always on)**: every drained batch is checked against the session's verification evidence (`agent/tools/taskflow/evidence_collector.py`); when the session has no passing evidence, a mandatory-verification message is appended after the carriers — the carriers themselves are injected verbatim. The lookup is fail-open: an unavailable evidence collector never blocks the turn.
