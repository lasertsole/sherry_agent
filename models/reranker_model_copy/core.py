...
class CrossEncoderGGUF:
    ...
    def __init__(
        ...
        # ── Model dimensions ────────────────────────────────────────
        self.n_embd = ll.llama_model_n_embd(self._model)
        ...
        # ── Classifier weights ──────────────────────────────────────
        logger.info("Loading classifier weights from GGUF …")
        self._cw = load_classifier_weights(self._model_path)

        # ── Enable embeddings output ──────────────────────────────
        # Ensure the context outputs embeddings via llama_get_embeddings_ith
        ll.llama_set_embeddings(self._ctx, True)

        # ── Tokeniser ───────────────────────────────────────────────
        self._vocab = ll.llama_model_get_vocab(self._model)
        self._cls_token = ll.llama_token_cls(self._vocab)
        self._sep_token = ll.llama_vocab_sep(self._vocab)
    ...
    def predict(self, query: str, passage: str) -> float:
        ...
        logit = compute_logit(hidden, self._cw)
    ...
    def rank(
        ...
        Mirror of ``RerankerModel.rank`` from ``models/reranker_model.core``.
        """
        if not documents:
            return []
    ...
    def filter(
        ...
        Args:
            query: The query string.
            documents: List of candidate document texts.
            gap_score: Minimum score threshold (0.0–1.0); defaults to 0.85.

        Returns:
            Document texts that score ≥ *gap_score*, in original order.

        Mirror of ``RerankerModel.filter`` from ``models/reranker_model.core``.
        """
        if not documents:
            return []
    ...
    def _encode_single(self, tokens: list[int]) -> npt.NDArray[np.float32] | None:
        ...
        Important: For llama.cpp 0.3.31, setting n_seq_id[i] > 0 causes heap
        corruption in llama_batch_free after llama_encode for BERT models.
        The fix: leave n_seq_id=0 (batch_init default) and DO NOT touch
        seq_id pointers at all. BERT encode does not need sequence IDs.
        """
        n_tokens = len(tokens)
        batch = ll.llama_batch_init(n_tokens, 0, 0)

        for i, tok in enumerate(tokens):
            batch.token[i] = tok
            batch.pos[i] = i
            batch.n_seq_id[i] = 0  # CRITICAL: n_seq_id is garbage-initialised by batch_init
            # Must explicitly set to 0, otherwise llama_encode may see stale values and fail.
            batch.logits[i] = 0
