 ...
 *
 * Design constraints:
 *  - Every action is a safe no-op when `import.meta.client === false` or when monitoring has not started.
 *  - stopConnectionWatch only removes this store's mitt subscriptions and window listeners; it never
...
export const useConnectionStore = defineStore('connection', () => {
   ...
   *
   * @returns Stop handle; calling it removes this store's mitt subscriptions and window listeners
   *   (does not close the WS singleton).
   */
   ...
   *
   * Note: this only removes this store's mitt subscriptions and window listeners; it does **not**
  ...
  /**
   * Test-only: reset the store singleton's private state (test isolation, never call from production code).
