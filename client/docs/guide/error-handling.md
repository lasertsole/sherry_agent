...
async function callWithRetry<T>(command: string, args: Record<string, unknown>, maxRetries = 3): Promise<T> {
...
function isRetryable(code: string): boolean {
  return ['IO_ERROR', 'CHANNEL_ERROR', 'MODEL_ERROR', 'DATABASE_ERROR', 'BACKEND_ERROR'].includes(code);
}
```
