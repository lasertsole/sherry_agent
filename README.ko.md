...
- **단기 세션 메모리**([MesMemory](context_engine/README.md)): 대화 이력을 SQLite(WAL 모드)에 영구 저장하고 FTS5 인덱스를 자동 생성 — 중국어 전문 검색용 trigram 토크나이저 테이블 포함; 각 모델 호출 경계마다 새 human/ai/tool 메시지를 증분 플러시합니다(`MessagePersistenceMiddleware`, `persisted_message_ids` 워터마크로 write-once). 원본 저장소는 더 이상 압축 발생에 의존하지 않습니다
