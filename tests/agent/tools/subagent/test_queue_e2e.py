...
pytestmark = [pytest.mark.asyncio, pytest.mark.timeout(120), pytest.mark.integration]
_TIMEOUT = 30.0
...
class _ScriptedStreamModel(BaseChatModel):
    ...
    - STRICT: raises _Provider400Error when the input transcript ends with an
    ...
    async def _astream(self, messages, stop=None, run_manager=None, **kwargs):
        ...
        if dangling:
            raise _Provider400Error(f"dangling tool_calls in model input: {sorted(dangling)}")
...
async def test_ac5_tool_park_cancel_heals(e2e_env):
    ...
    # raises _Provider400Error on dangling tool_calls - reaching the asserts below
