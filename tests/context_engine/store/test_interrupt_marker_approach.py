...
registers system_prompt_injection / MultimodalProcessor / IterationBudget /
...
class SimulatedProvider400Error(Exception):
...
class StrictProviderFakeChatModel(RecordingFakeChatModel):
    ...
    def _generate(
        ...
        **kwargs: Any,
        ...
        if dangling:
            raise SimulatedProvider400Error(
...
async def test_fact_b_tool_call_no_middleware_strict_provider_400():
    ...
    await _seed_dangling_state(graph, _config())
    with pytest.raises(SimulatedProvider400Error) as exc_info:
        await graph.ainvoke(_input([HumanMessage("Q2")]), _config())
