import pytest
from agent.tools.subagent.session.metrics import (
...
class TestMetrics:
    ...
    def test_runtime_running(self):
        ...
        run = SubagentRunRecord(
            ...
            execution=ExecutionState(
                status=ExecutionStatus.RUNNING, started_at=time.monotonic() - 5.0
            ),
        )
    ...
    def test_runtime_completed(self):
        run = SubagentRunRecord(
            ...
            task="t",
            execution=ExecutionState(
                status=ExecutionStatus.TERMINAL, started_at=100.0, ended_at=105.0
            ),
        )
        ms = get_subagent_session_runtime_ms(run)
