from agent.tools.subagent.registry.completion import (
    should_update_run_outcome,
    resolve_finalized_task_state,
    resolve_lifecycle_outcome,
)
from agent.tools.subagent.registry.cleanup import (
    ...
    def test_resolve_lifecycle_outcome(self):
        run = SubagentRunRecord(
            ...
            execution=ExecutionState(
                outcome=RunOutcome(status=RunOutcomeStatus.ERROR, error="fail")
            ),
        )
