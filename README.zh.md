...
### 1. 🧠 深度记忆系统 (Context Engine + 经验知识图谱)
- **双记忆架构**：短期对话记忆 [MesMemory](context_engine/README.zh.md) + 长期经验知识图谱
- **经验知识图谱**：蒸馏优先的知识图谱，从任务执行中提取高信噪比可复用经验，存为结构化节点和边
...
- ▶️ _详见 [Context Engine 文档](context_engine/README.zh.md) 和经验知识图谱文档_
...
- **经验闭环**：子代理通过 draft 工具记录发现 → 任务结束后蒸馏经验写入经验知识图谱 → 下次任务召回注入
