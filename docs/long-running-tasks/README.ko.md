...
| 부분 | 내용 |
| :--- | :--- |
| `config/features/agent_side/` | **26**개 에이전트 측 설정 모듈(미들웨어, 도구, LLM 클라이언트, 메모리, TaskFlow) |
...
레지스트리는 현재 **45개 feature 객체**를 보유합니다 — 에이전트 측 26 + 인프라 측 19 — 각 패키지 `__init__.py`를 통해 재수출되고 `config/features/__init__.py`가 집계하므로, 소비자는 절반 또는 전체 레지스트리를 한 곳에서 임포트할 수 있습니다. 소비 코드는 상수를 임포트해 직접 인덱싱합니다(예: `ITERATION_BUDGET["default_max_iterations"]`). `get_feature`/`load_feature` 접근자는 없습니다. `config/__init__.py:38-39`는 `GATEWAY`에서 `API_HOST`/`API_PORT`를 파생합니다.
...
| `taskflow_resume` | `(flow_id, child_session_key="", result="", expected_revision=None, token_usage=None, validation_criteria=None, session_id)` | 재개 상태, 언락된 단계, 단계 상태 카운트, 기준 에코, 판정기 판정, 재시도 노트 |
...
| `taskflow_finish` | `(flow_id, summary="", expected_revision=None, todo=None, plan_path=None, checkbox_label=None, session_id)` | 종단 `done`(4개 게이트, 페일오픈) |
...
| `deps_satisfied` / `unlock_dependents` | `agent/tools/taskflow/tools/_shared.py:115,153` | DAG 전환 |
...
| `plan_settled_retries` / `persist_retry_actions` | `agent/tools/taskflow/tools/_retry.py:218,273` | wait_all 재시도 계획/영속화 |
| `_backflow_shared_memory` | `agent/middlewares/subagent_completion_drain/core.py:93` | 메모리 역류 조정 |
| `judge_step_result` | `agent/tools/taskflow/step_judge.py:142` | 단계별 pass/retry/block 판정 |
...
| `test_validation.py` | 기준 저장, 판정기 판정(`pass`/`retry`/`block`), 덮어쓰기, 페일오픈 |
| `test_step_judge.py` | StepJudge 응답 파싱, 판정 정규화, `pass`로의 페일오픈, 프롬프트 조립 |
| `test_resume_with_judge.py` | `taskflow_resume` + 판정기: pass/retry/block, 재시도 피드백 주입, 예산 소진, 페일오픈 |
| `test_evidence_collector.py` | 판정 프롬프트용 evidence 요약 렌더링; staleness는 이후 `stale` 행에서 파생 |
| `test_finish_gate.py` | 완료 게이트 A–D: DAG 완전성, blocked 단계, 실패/stale evidence, `SisyphusVerifier` |
| `test_chain_smoke.py` | 판정기 → 재시도 피드백 → goal loop → 완료 게이트 통합 스모크(stub 보조 LLM) |
| `test_taskflow_list.py` | 세션 보드 렌더링, 상태 필터, 마지막 활동 타임스탬프 |
| `test_index_audit.py` | SQLite 인덱스 업그레이드 경로와 쿼리 플랜 감사 |
| `test_update_steps.py` | 단계 목록 전면 교체: 추가/삭제/재작성/재정렬과 dispatched/done 안전 규칙 |

교차 스위트: `tests/agent/middlewares/test_memory_flush.py`(플러시 임계값과 `append_entries`), `tests/agent/middlewares/test_lt5_memory_backflow.py`(완료 배출 시 메모리 조정), `tests/agent/middlewares/test_subagent_completion_drain_reminder.py`(완료 캐리어 검증 리마인더), `tests/agent/middlewares/test_completion_drain_gate.py`(옵트인 `enforce_verification` 프로그램 게이트), `tests/agent/tools/subagent/test_completion_judge.py` + `test_goal_loop.py`(완료 판정기와 유계 goal loop), `tests/agent/tools/test_evidence_auto_record.py` + `test_evidence_stale.py` + `tests/agent/tools/todolist/test_evidence_ledger.py`(evidence 기록, stale 이벤트, 원장 뷰), `tests/context_engine/test_session_continuity.py`(연속성 저장/프롬프트), `tests/agent/middlewares/test_todo_continuation.py`(턴 종료 연속), `tests/pub/func/message/test_tool_output_prune.py`(한 줄 요약), `tests/workspace/test_prompt_builder_taskflow.py`(보류 flow 프롬프트 주입).
...
- **레지스트리 규모는 45입니다.** 설정 레지스트리는 45개 feature 객체(에이전트 측 26 + 인프라 측 19)를 보유합니다. 인프라 측 계약 테스트는 그중 18개(GATEWAY + 17개 데이터 기반 케이스)를 커버하고 `MODEL_PRICING`을 빠뜨립니다.
...
- **기준이 있으면 결과 검증은 판정기가 수행합니다.** 기준을 가진 단계는 재개 시 보조 LLM 단계 판정기가 심사합니다: `retry`는 단계 자체의 재시도 예산 안에서 재디스패치하고, `block`(또는 예산 소진)은 단계를 `blocked`로 표시합니다. 판정기는 페일오픈이므로 비활성/사용 불가 시 `pass`로 강등됩니다 — 결과가 기준을 충족했다는 강한 보장은 없고, 단계 수용 전의 최선 노력 판정일 뿐입니다.
