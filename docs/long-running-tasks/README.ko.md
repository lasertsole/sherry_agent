...
교차 스위트: `tests/agent/middlewares/test_memory_flush.py`(플러시 임계값과 `append_entries`), `tests/agent/middlewares/test_lt5_memory_backflow.py`(완료 배출 시 메모리 조정), `tests/agent/middlewares/test_subagent_completion_drain_reminder.py`(완료 캐리어 검증 리마인더), `tests/agent/middlewares/test_completion_drain_gate.py`(무조건적 프로그램 게이트), `tests/agent/tools/subagent/test_completion_judge.py` + `test_goal_loop.py`(완료 판정기와 유계 goal loop), `tests/agent/tools/test_evidence_auto_record.py` + `test_evidence_stale.py` + `tests/agent/tools/todolist/test_evidence_ledger.py`(evidence 기록, stale 이벤트, 원장 뷰), `tests/context_engine/test_session_continuity.py`(연속성 저장/프롬프트), `tests/agent/middlewares/test_todo_continuation.py`(턴 종료 연속), `tests/pub/func/message/test_tool_output_prune.py`(한 줄 요약), `tests/workspace/test_prompt_builder_taskflow.py`(보류 flow 프롬프트 주입).
...
- **기준이 있으면 결과 검증은 판정기가 수행합니다.** 기준을 가진 단계는 재개 시 보조 LLM 단계 판정기가 심사합니다: `retry`는 단계 자체의 재시도 예산 안에서 재디스패치하고, `block`(또는 예산 소진)은 단계를 `blocked`로 표시합니다. 판정기는 페일오픈이므로 모델 오류나 파싱 불가 응답은 `pass`로 강등됩니다 — 결과가 기준을 충족했다는 강한 보장은 없고, 단계 수용 전의 최선 노력 판정일 뿐입니다.
