...
| **Knowledge** | 계획 소유권 단위로 격리(도구 계층에서 강제): `ownership.is_plan_associated()`가 세 소스의 합집합을 사용합니다 — 세션의 `plan_ref` 상태 키, 세션 todo의 `plan_ref`(SQL에서 `session_id`로 필터), `.omo/boulder.json`에서 `plan_name`이 일치하고 `session_ids`에 해당 세션을 포함하는 work. 다른 세션의 계획에 대한 `read` / `write`는 진단 가능한 오류로 거부되고 `list`는 연결된 계획만 반환합니다; 여러 세션이 공유하는 계획은 모든 세션에서 계속 사용할 수 있습니다. `build_knowledge_block(session_id)`는 프롬프트 블록을 위해 해당 세션의 `plan_ref`를 해석합니다. | `agent/tools/todolist/knowledge/ownership.py` |
...
- **Knowledge는 계획 소유권 단위로 격리되며 이름 채택으로 공유되지 않습니다.** `knowledge` 도구는 계획 이름을 받고, 접근은 `ownership.is_plan_associated()`가 제어합니다: 세션의 `plan_ref`, todo의 `plan_ref`, 또는 `session_ids`에 해당 세션을 포함하는 boulder work. 같은 계획 이름을 채택한 것만으로 두 세션이 그 계획의 지식 디렉터리를 공유하지는 않습니다 — 하지만 boulder `session_ids`로 명시적으로 공유된 계획(다중 세션 협업)은 나열된 모든 세션이 읽고 쓸 수 있습니다. 서브에이전트 경계는 그대로 절대적입니다 — `knowledge`는 `main_only`입니다.
