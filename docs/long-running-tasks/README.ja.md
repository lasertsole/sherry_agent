...
| **Knowledge** | 計画オーナーシップ単位で分離（ツール層で強制）：`ownership.is_plan_associated()` が 3 ソースの和集合を取ります——セッションの `plan_ref` 状態キー、セッション todo の `plan_ref`（SQL で `session_id` をフィルタ）、`.omo/boulder.json` のうち `plan_name` が一致し `session_ids` に当該セッションを含む work。他セッションの計画への `read` / `write` は診断可能なエラーで拒否され、`list` は関連付けられた計画のみを返します；複数セッションで共有された計画は全セッションで引き続き利用できます。`build_knowledge_block(session_id)` はプロンプトブロック用にそのセッション自身の `plan_ref` を解決します。 | `agent/tools/todolist/knowledge/ownership.py` |
...
- **Knowledge は計画オーナーシップ単位で分離され、名称の採用では共有されません。** `knowledge` ツールは計画名を取り、アクセスは `ownership.is_plan_associated()` が制御します：セッションの `plan_ref`、todo の `plan_ref`、または `session_ids` に当該セッションを含む boulder work。同じ計画名を採用しただけの 2 セッションはその計画の知識ディレクトリを共有しなくなりました——ただし boulder `session_ids` で明示的に共有された計画（複数セッション協業）は、列挙された全セッションが読み書きできます。サブエージェント境界は絶対的のままです——`knowledge` は `main_only` です。
