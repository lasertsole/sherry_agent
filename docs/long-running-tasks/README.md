...
| **Knowledge** | per plan ownership, enforced in the tool: `ownership.is_plan_associated()` unions the session's `plan_ref` state key, the session's todo `plan_ref`s (SQL filtered by `session_id`) and `.omo/boulder.json` works whose `plan_name` matches and whose `session_ids` contain the session. `read` / `write` on a foreign plan are refused with a diagnosable error and `list` returns only associated plans; a plan shared by several sessions stays usable by all of them. `build_knowledge_block(session_id)` resolves the session's own `plan_ref` for the prompt block. | `agent/tools/todolist/knowledge/ownership.py` |
...
- **Knowledge is isolated per plan ownership, not by name adoption.** The `knowledge` tool takes a plan name, and access is gated by `ownership.is_plan_associated()`: the session's `plan_ref`, a todo `plan_ref`, or a boulder work whose `session_ids` include the session. Two sessions that merely adopt the same plan name no longer share that plan's knowledge directory — but a plan explicitly shared through boulder `session_ids` (multi-session collaboration) remains readable and writable by every listed session. The subagent boundary stays absolute — `knowledge` is `main_only`.
