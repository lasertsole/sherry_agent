...
可用性は 3 状態で報告されます：`available`、`not_installed`（設定済みだが発見を通過するバイナリがない——インストールヒントとローカルインストールコマンドを返す）、`not_configured`（言語が `lsp_enabled_languages` にない）。利用不能な経路はすべてツール名、インストールヒント、そして `explore` または `search_files` へのフォールバックを返します。自動インストールは既定で無効（`lsp_auto_install=False`）で、有効時も対象言語の許可リスト済みコマンドだけを 60 秒のタイムアウトと洗浄済み環境で実行します。
...
これらの能力を欠くマシンは失敗せず降格します：ast-grep が解決できなければ検証付き自動プロビジョニング経路が起動し、埋め込みバックエンドが利用不能なら `semantic_code_search` は `degraded` メッセージを返して `explore` と `search_files` を指します。
...
- **`search_files` は `_MAIN_TOOLS_BUILDERS` に登録されていません。** librarian 定義はそれを列挙し、LSP フォールバック文言も言及しますが、そのビルダーは候補集合にないため、librarian の実際の検索経路は `terminal` と `explore`（および他の code-intel ツール）です。
