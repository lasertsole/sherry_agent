...
| 멱등 무진전 | 동일한 멱등 도구가 그 턴에서 이미 반환한 결과 해시를 반환 | 2 | 5 | 5에서 HALT |
| 핑퐁 | 끊김 없는 A → B → A → B 왕복으로, 연속된 두 호출이 모두 무진전 | 4 | 6 | 6에서 HALT |
| 인자 갱신 | 같은 멱등 도구가 결과가 변하지 않는 인자 변형을 순환 | 3개 변형 | 5개 변형 | 5에서 HALT |

중요한 세부 사항:

- **복구 모드** (`recovery_mode_enabled=True` 기본값): 첫 BLOCK은 턴을 벽돌로 만들지 않습니다. 턴은 복구 상태로 들어가고, *precheck* 경로가 차단된 도구를 풀어주어 재시도가 새로 평가됩니다. 이후 BLOCK마다 위반 카운터가 증가하고, 카운터가 `recovery_max_violations`(기본 1)를 초과하면 HALT로 격상됩니다. 요컨대 즉석 벽이 아니라 관리되는 재시도 창입니다. 엄격한 비복구 동작을 원하면 `recovery_mode_enabled=False`로, 모든 BLOCK 임계값을 HALT로 바꾸려면 `hard_stop_enabled=True`로 설정하세요(표 참조).
...
`agent/tools/taskflow/step_judge.py`는 온도 0의 보조 LLM 판정기로, 디스패치된 단계의 결과를 `validation_criteria`에 비추어 심사하고 `pass` / `retry` / `block`을 반환합니다. 단계가 기준을 가질 때(`taskflow_run_task` 또는 `taskflow_resume`가 설정)만 작동하므로 평범한 단계가 판정 호출 비용을 치르지 않습니다.
...
| 게이트 | 검사 | 거부 사유 |
|---|---|---|
| **A — DAG 완전성** | 모든 단계가 `done` 또는 `blocked` | `step(s) not done/blocked` |
| **B — 차단 없음** | 어떤 단계도 `blocked`가 아님 | `step(s) are blocked` |
| **C — 흐름 증거** | 흐름 증거에 `FAIL`도 `[stale]` 행도 없음 | `failing verification evidence` / `stale evidence` |
| **D — SisyphusVerifier** | 플랜 재독, 수용 기준, 연결된 `done` 단계 — 호출자가 `todo`와 `plan_path`를 모두 넘길 때만 | `SisyphusVerifier` 사유 |
