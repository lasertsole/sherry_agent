...
| `librarian` | 읽기 전용 외부 코드베이스 검색: 서드파티 저장소 clone·색인·검색 | `auxiliary` | `read_file`, `terminal`, `web_search`, `search_files` + 코드 인텔리전스 `explore`, `callers`, `callees`, `impact`, `semantic_code_search` + LSP `lsp_goto_definition`, `lsp_find_references`, `lsp_workspace_symbol`, `lsp_call_hierarchy`, `lsp_rename`, `lsp_diagnostics`, `lsp_format`, `lsp_status` + ast-grep `ast_grep_search`, `ast_grep_rewrite` |
