# 🧱 Isolation Capabilities & Precedence

[**English**](README.md) · [**中文**](README.zh.md) · [**한국어**](README.ko.md) · [**日本語**](README.ja.md)

> Part of [Tool Sandbox](../README.md): the isolation capabilities (L1 env scrubbing, L2 OS-native backends, the command and sensitive-file gates, the HITL bypass approval, and the file-tool path gates) plus the authoritative precedence matrix.
...
**Scan bounds.** Both modes also bound the scan itself through `TOOLS_TIMEOUTS` (`config/features/agent_side/tools_timeouts.py`): `file_tools_search_time_budget_s` (default 5.0 s) stops the walk when it expires, `file_tools_search_max_matches` (default 10,000) caps the hits collected, and `file_tools_search_prune_dirs` (default `proc`, `sys`, `dev`) is filtered out of `dirnames[:]` so pseudo-filesystems are never descended. A cut scan is never silent: the JSON result gains `scan_truncated: true`, `scan_stop_reason` (`time_budget` / `max_matches`) plus a hint, and `pruned_dir_count` when directories were pruned. File-name patterns go through `fnmatch` (no brace expansion), so no expansion cap is needed.
