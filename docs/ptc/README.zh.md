...
> 本文是[子代理设计页](../subagent/README.zh.md)（让 `execute_code` 仅归 executor 的两轴角色模型）、[子代理系统 README](../../agent/tools/subagent/README.zh.md)（角色表中列出该工具的运行时与 API 参考）与[工具沙箱页](../sandbox/README.zh.md)（`terminal`、`python_repl` 与 PTC 的 `execute_code` 所解析的隔离栈）的设计层姊妹篇。本页记录 PTC 是什么、脚本为何在独立进程中执行、它如何抵达真实工具、这层约束真正保证了什么，以及在哪些地方它诚实地弱于 OS 沙箱。

事实来源：`agent/tools/ptc/**`、`config/features/agent_side/ptc.py`、`config/features/agent_side/tools_timeouts.py`、`agent/tools/subagent/types/functional_role.py`、`agent/tools/subagent/spawn/core.py`、`agent/tools/subagent/spawn/system_prompt.py`、`agent/tools/pub_base/env_scrub.py`、`agent/tools/pub_base/sandbox.py` 与 `tests/agent/tools/ptc/**`。下文每一条陈述均已对照该源码核实。
...
| 返回什么？ | 一个 JSON 信封，含 `status`（`ok` / `timeout` / `error` / `budget_exceeded` / `sandbox_unavailable`）、`output`、`error`、`exit_code` 与 `tool_calls_made` |
...
**与 `python_repl` 的分工。** 两个执行面都以镜像的受限 builtins 集合与同一个 `scrub_env` 执行模型编写的 Python，也都把子进程的启动目录设为 `ROOT_DIR`。它们的形态不同：`python_repl` 在 30 秒预算内运行单个代码片段且没有任何工具访问；`execute_code` 则运行更长的脚本（120 秒预算），可以同步调用最多 50 次真实工具。正是这份额外的触达使 PTC 仅限 executor，也正是它额外引入进程组、RPC 桥、调用预算与导入白名单的原因。executor 角色保留 `python_repl`；PTC 是增量能力，不是替代品。两者也解析同一套 OS 原生沙箱后端：PTC 子进程的 argv 会被完全按 `sandbox=True` 的 `python_repl` 调用那样包装（见[工具沙箱页](../sandbox/README.zh.md)），`auto` 降级与 `required` 拒绝的语义见[限制](#-限制)。
...
| `runner.py` | `run_ptc` 编排单次调用：启动 RPC 服务、创建临时目录、生成 `sherry_tools.py`、写入包装脚本、生成一次性 RPC token、用 OS 原生沙箱后端包装子进程 argv、派生子进程、捕获并截断输出，最后清理 |
| `rpc_server.py` | loopback TCP 监听器：按行解析 JSON 请求、用每次运行的 token 校验每一帧、执行每脚本调用预算，并把每次调用派发到父事件循环 |
| `stub_generator.py` | 从每个工具的 `tool_call_schema` 推导 `ToolStub` 规格，并渲染 `sherry_tools.py` 模块——每个工具一个同步包装器，外加本地辅助函数；RPC 调用从 `SHERRY_PTC_RPC_TOKEN` 读取 token，而不是把 token 嵌入其中 |
| `builtins.py` | 持有子进程的精简 builtin 命名空间与导入白名单，并把两者渲染进包装脚本 |

线协议是双向的、每行一个 JSON 对象：

```json
{"tool": "read_file", "args": {"file_path": "README.md"}, "token": "<一次性 token>"}
{"ok": true, "result": "..."}
...
**产物与生命周期。** 每次调用都会获得全新的 `sherry_ptc_*` 临时目录，内含 `sherry_tools.py` 与包装脚本；子进程唯一的路径增补是 `PYTHONPATH=<tmpdir>`。子进程环境还携带本次运行的一次性 RPC token（`SHERRY_PTC_RPC_TOKEN`），生成的 stub 读取它并在每次请求中发送；token 由 `secrets.token_urlsafe` 生成，仅内存存活，绝不写入 `sherry_tools.py` 或任何其它文件。当后端可用时，子进程 argv 还会经 `backend.wrap([sys.executable, script_path], env)` 包装（即[工具沙箱](../sandbox/README.zh.md)中的沙箱路径）；进程仍以 `start_new_session=True` 派生，超时即对整组发 SIGKILL。包装器把用户脚本的 stdout 与 stderr 捕获进缓冲区，向真实 stdout 打印单个 JSON 信封，runner 据此归类出 `status` 字段。`finally` 块停止 RPC 服务、join 其线程、删除临时目录，并在子进程意外存活时再次击杀。
...
1. **环境清洗。** 子进程环境从 `scrub_env()` 出发：名称含 `KEY`、`TOKEN`、`SECRET`、`PASSWORD`、`CREDENTIAL`、`PASSWD`、`AUTH`、`DSN`、`WEBHOOK`、`BEARER` 或 `APIKEY`（大小写不敏感）的变量被丢弃，Sherry 自身的 `*_API_KEY` 变量被显式拒绝，而 `PATH` 等关键名称依照精确名称优先级保留。随后 runner 只增补 `PYTHONPATH`、`PYTHONUNBUFFERED` 与本次运行的一次性 RPC token（`SHERRY_PTC_RPC_TOKEN`）。
...
7. **RPC 仅绑 loopback 且要求一次性 token。** 监听器在 `127.0.0.1` 上绑定临时端口；构造函数直接拒绝 `0.0.0.0`、`::` 与空主机，且在把地址交给子进程之前会再次核验。每一帧请求还必须携带子进程从 `SHERRY_PTC_RPC_TOKEN` 读取的一次性 token；服务端以常量时间比较，缺失或错误的 token 会让它立即关闭连接、不回任何响应，因此游离的本机进程既无法调用工具，也无法探知猜测是否接近。
...
- **它本身并不是 OS 级沙箱。** 受限命名空间与导入守卫约束的是字面脚本能指称的名字，但 `getattr` 及其他内省 builtins 仍然可用，因此有决心的脚本可以沿可达对象继续走，触及其字面表面之外的更多能力。这属于受限命名空间层级——与既有 `python_repl` 包装器 builtins 同级。在此之上，只要存在可用的后端，子进程 argv 现在就会被 `terminal` 与 `python_repl` 所解析的同一套 OS 原生沙箱后端（Linux 的 bubblewrap、macOS 的 Seatbelt）包装，从而获得它们的写包含与敏感路径读屏蔽。仍有两个诚实的提醒：`SANDBOX_POLICY=auto`（默认）且无可用后端时，运行会在恰好一条 loguru 警告后降级为无沙箱；而 bwrap 构造使用了 `--unshare-all`，它同时隔离网络命名空间——在真实 bwrap 下 loopback RPC 桥是否仍可达，未在真实 Linux 机器上验证。后端本身见[工具沙箱](../sandbox/README.zh.md)。
...
- **RPC token 不是跨用户边界。** 监听器现在要求一次性 token，因此读不到它的本机进程既无法调用工具，也无法完成请求。但 token 经由子进程环境传递，因此任何能在脚本短暂生命周期内读取 `/proc/<pid>/environ` 的*同用户*进程仍可恢复它；该 token 防的是无关的本机进程，而不是同用户攻击者。监听器也仍然只绑 loopback，且只存活于单个脚本的执行期间。
...
| `required` 但沙箱不可用 | `status: "sandbox_unavailable"`，错误点名 `SANDBOX_POLICY=required` | 在派生子进程之前即拒绝该次运行；父回合收到可操作提示：安装 bubblewrap / 设置 `SANDBOX_POLICY=auto` |
...
| RPC token 握手：正确 / 缺失 / 错误三态，拒绝时立即关闭连接 | `tests/agent/tools/ptc/test_rpc_auth.py` |
| 沙箱策略映射：后端包装、`required` 拒绝、`auto` 降级并恰好一条警告、`off` | `tests/agent/tools/ptc/test_sandbox_integration.py` |
| runner 加固：每次运行的 token 唯一性、生成的 stub 文件不含 token、子进程 argv 被后端包装、唯一那条降级警告 | `tests/agent/tools/ptc/test_runner_hardening.py` |
...
| [工具沙箱](../sandbox/README.zh.md) | 环境清洗，以及 `terminal`、`python_repl` 与 PTC 的 `execute_code` 所解析的 OS 原生隔离——包括 PTC 继承的 `auto` 降级与 `required` 拒绝 |
