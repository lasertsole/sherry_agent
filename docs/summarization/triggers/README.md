...
turn starts
│
├─ T1  before_agent preflight  (_t1_preflight overflow.py:795 / _at1_preflight overflow.py:820)
│      ├─ _reset_turn_state (thrash.py:157) resets the 11 per-turn counters
│      ├─ _decide_overflow_route (overflow.py:185) → None / "fits" → no-op
│      ├─ cooldown > 0 blocks the COMPACT routes; the truncate track
│      │  still runs (it is the cheap recovery mechanism itself)
│      └─ dispatch (trigger="T1") + _t1_state_update (overflow.py:773) commits the
│         result to the graph:
...
│
├─ T2  wrap_model_call, pre-handler (core.py:413 sync / core.py:495 async)
│      ├─ read force flag (core.py:426) BEFORE the skip gate — the skip gate
│      │  (_should_skip_compression thrash.py:110) consumes the flag
│      ├─ _tick_cooldown (thrash.py:87): EVERY call decrements the cooldown
│      ├─ anti-thrash gate (core.py:437–439):
...
│      │      happened, core.py:441–459) → handler → monitor → T3
│      ├─ else: 4-route decision (core.py:472) → _dispatch_overflow_route;
│      │  elif a legacy trigger clause fires (_check_trigger overflow.py:135,
│      │  e.g. ("messages", 40)) → ROUTE_COMPACT_ONLY (core.py:480)
│      └─ the handler call itself runs inside
│         _execute_with_recovery (overflow.py:703) — the T4/T5 ring
│
├─ T3  post-response re-check  (_post_response_check overflow.py:511 / async overflow.py:543)
│      ├─ skipped when T2 compressed in THIS wrap call (t2_compressed
│      │  flag, core.py:484–486) — exactly one compression per model call
│      ├─ extract_reported_input_tokens(response) (overflow.py:66); None → return
...
└─ T4/T5  provider-error recovery ring
       ...
       │  (overflow.py:625 / async overflow.py:662): the no-LLM tail clip runs FIRST — when it
       ...
       │  the per-class retry counter increments AFTER success (overflow.py:614)
...
The legacy trigger clauses still exist as the T2 fallback (`_check_trigger`, overflow.py:135): `("messages", N)` fires on history length, `("tokens", N)` fires on `max(local estimate, last AIMessage's reported usage_metadata.total_tokens)` ≥ N. A clause list is an OR.
...
The single executor `_dispatch_overflow_route` (overflow.py:408 sync / overflow.py:430 async) serves T1, T2 **and** T3 — never a second copy:

- `truncate_tool_results_only` → `_run_budget_truncation` (overflow.py:219) — step 1 truncates oversized tool-call args (returns new messages, see the [truncate track](../internals/README.md#-the-truncate-track-budget-truncation--the-ttl-module)), step 2 truncates tool results in place — then a **recheck**: if the freed tokens were not enough (`new_tokens ≥ usable × 0.80`, estimated on the returned list), escalate to `compact_then_truncate`; otherwise pass through WITHOUT compression;
- `compact_only` / `compact_then_truncate` → `_execute_compact` (overflow.py:326 / async overflow.py:344) → `_apply_compression` (exceptions logged, request unchanged) → `_record_compaction_bookkeeping` (core.py:345: arm the cooldown, count the turn attempt) → for `compact_then_truncate`, budget truncation runs on the compacted result as backstop → route logged with old/new tokens and pressure ratio.
