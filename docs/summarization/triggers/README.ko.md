...
- `truncate_tool_results_only` → `_run_budget_truncation`(:659) — 1단계는 과도하게 큰 도구 호출 인자를 잘라내고(새 메시지 반환, [트렁케이트 트랙](../internals/README.ko.md#-트렁케이트-트랙-예산-트렁케이션과-ttl-모듈) 참조), 2단계는 도구 결과를 제자리에서 잘라냄 — 이후 **재확인**: 확보한 토큰이 부족하면(`new_tokens ≥ usable × 0.80`, 반환된 목록 기준으로 추정) `compact_then_truncate`로 승격; 아니면 압축 없이 통과;
