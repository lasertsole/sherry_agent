...
Message persistence has left the compression path: every model boundary flushes the new human/ai/tool messages to MesMemory through `MessagePersistenceMiddleware` (`agent/middlewares/message_persistence/`), write-once via the persistent `persisted_message_ids` watermark. The compression-time flush module (`compaction_persistence.py`) and its `_persist_discarded_messages_sync` / `_apersist_discarded_messages` call sites were removed — a compact now only compacts and schedules the nudge dispatch below. See `agent/middlewares/README.md` for the middleware's trigger semantics.
...
| `tests/agent/middlewares/message_persistence/` | 16 | Per-model-boundary flush: each message exactly once, no duplicate across boundaries, restart replay via the persistent watermark, sync + async hooks, missing-session_id skip, HITL denial re-pairing, filter semantics; plus T1/T2/T3 compactions proving zero store writes |
