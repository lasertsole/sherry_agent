...
2. **Non-LLM strategies** (`_run_non_llm_strategies`, :1493): `dedup → prune → target truncate → tool-args truncate` (details below). These are free — no model call.
...
### 💾 Pre-replacement persistence & compression-time nudges

When a compact actually discards a prefix (`cutoff > 0`), two compression-side side effects run **before** the replacement messages are built:

1. **Discarded-prefix flush** (`agent/middlewares/summarization/compaction_persistence.py`, called through `_persist_discarded_messages_sync` / `_apersist_discarded_messages`): the messages being removed are flushed to MesMemory before `_build_new_messages` / `request.override` swap in the summary pair. The flush takes the **original** message objects from `request.state["messages"]` — never the post-`_run_non_llm_strategies` copies, whose tool outputs may already be deduplicated or truncated. The discarded slice is located by object identity (`preserved[0]` found in the original list by `id()`); when a strategy replaced that object, the fallback keeps every original whose `id()` is absent from `preserved`. HITL denial ToolMessages are re-paired onto their AI message first; duplicate and empty tool results are dropped. Any store error is logged and compression proceeds — the flush is fail-open.
2. **Compression-time nudges** (`agent/middlewares/context_engine/nudge.py::schedule_compression_nudges`): the memory-review counter (`nudge_review_memory_count`, `state_register_db`) increments once per compression and fires `_nudge_memory` at `nudge_memory_threshold` (default 10); plan extraction evaluates `_detect_todo_all_complete` at the same point. Both dispatch fire-and-forget under the NUDGE lane, so they can never block the model call. The two triggers previously ran from `ContextEngineHook.after_agent` on every turn; that hook no longer exists. The single-fire `nudge_plan_extraction_fired` flag is unchanged — one extraction per completion cycle — so a session that never compresses never fires plan extraction.

The write-once guarantee: messages carrying the in-process `_db_persisted` marker and messages recorded in the persistent `persisted_message_ids` watermark (a `(session_id, message_id)` tombstone table in `mes_memory.db`) are skipped. Graph-state message ids survive checkpoint serialization, so a T2 flush (request-only override; state still holds the slice) followed by a T1 flush, or a checkpoint replay after a process restart, never inserts a duplicate row.
...
2. **Chain the prior checkpoint** (`_extract_previous_summary`, :1376): the newest `AIMessage` with `additional_kwargs["lc_source"] == "summarization"` is found and its `<summary>…</summary>` body extracted. If present, the prompt becomes `conversation + prior-summary + _SUMMARY_PROMPT_UPDATE` (:245) instead of `_SUMMARY_PROMPT_FIRST` (:237) — carry objectives/constraints/decisions forward, newest wins conflicts, FIFO limits.
...
| `tests/agent/middlewares/test_compaction_persistence.py` | 9 | Discarded-prefix flush: T2→T1 and restart write-once with exact row counts, original tool content vs non-LLM rewrites, flush-before-replacement ordering, compression-time nudge dispatch, sync + async paths |
