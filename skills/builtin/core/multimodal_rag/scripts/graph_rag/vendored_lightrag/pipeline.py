...
from graph_rag.vendored_lightrag.base import DocProcessingStatus, DocStatus
...
from graph_rag.vendored_lightrag.kg.shared_storage import get_namespace_data, get_namespace_lock
from graph_rag.vendored_lightrag.operate import merge_nodes_and_edges
from graph_rag.vendored_lightrag.parser.base import ParseContext
from graph_rag.vendored_lightrag.parser.registry import (
    get_parser,
    parser_specs_snapshot,
    supported_parser_engines,
    suffix_capabilities,
)
from graph_rag.vendored_lightrag.parser.routing import (
    parser_suffix,
...
from graph_rag.vendored_lightrag.utils_pipeline import (
...
class _PipelineMixin:
    ...
    async def apipeline_enqueue_documents(
        ...
        def _process_options_at(index: int) -> str:
            if process_options is None:
                return ""
            from graph_rag.vendored_lightrag.parser.routing import sanitize_process_options
    ...
    async def process_single_document(
        ...
        *,
        ...
        """
        from graph_rag.vendored_lightrag.parser.routing import parse_process_options
        ...
        async with ctx.semaphore:
            try:
                ...
                if doc_process_opts.chunking_explicit:
                    from graph_rag.vendored_lightrag.chunker import (
            ...
            if source_name:
                for candidate in matches:
                    if candidate.name == source_name:
                        return str(candidate)
            if parser_engine:
                from graph_rag.vendored_lightrag.parser.routing import filename_parser_directives
    ...
    async def analyze_multimodal(
        ...
        *,
        ...
        try:
            ...
            from graph_rag.vendored_lightrag.llm._vision_utils import (
                image_audit_metadata,
                image_cache_metadata,
                normalize_image_inputs,
                read_image_dimensions,
            )
            from graph_rag.vendored_lightrag.prompt_multimodal import (
                IMAGE_TYPE_ENUM,
                IMAGE_TYPE_FALLBACK,
                MULTIMODAL_PROMPTS,
