...
from ..base import DocProcessingStatus, DocStatus, DocStatusStorage
...
class SNKVDocStatusStorage(DocStatusStorage):
    ...
    async def get_docs_paginated(
        ...
        # Clamp page_size to the contract's [10, 200] range as in 1.5.2
    ...
    async def get_doc_by_file_basename(self, basename: str) -> tuple[str, dict[str, Any]] | None:
        ...
        the business layer. We match on the basename component of ``file_path``
        and, as a fallback, an exact match when ``file_path`` has no path prefix.
        """
        ...
        def _find():
            for doc_id, doc in self._iter_all():
                if not doc.file_path:
                    continue
                candidate = os.path.basename(doc.file_path.replace("\\", "/"))
                if candidate == basename or doc.file_path == basename:
                    d = _to_dict(doc)
                    d["id"] = doc_id
                    return (doc_id, d)
            return None
