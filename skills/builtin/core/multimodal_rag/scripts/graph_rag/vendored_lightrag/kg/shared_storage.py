...
from graph_rag.vendored_lightrag.exceptions import PipelineNotInitializedError
...
class KeyedUnifiedLock:
    ...
    @staticmethod
    def _get_running_loop() -> object | None:
        """Get the currently running event loop, or None if none is running."""
        try:
    ...
    def _get_or_create_async_lock(self, combined_key: str) -> asyncio.Lock:
        ...
        # Validate that the cached lock is bound to the *currently running*
        # event loop.  When nest_asyncio is active or event loops are
        # swapped, a lock created on loop A may silently fail when used on
        # loop B with a cryptic "different event loop" RuntimeError.
        # If the loop changed, discard the stale entry and create a fresh
        # lock for the current loop.
        current_loop = self._get_running_loop()
        if current_loop is not None and async_lock is not None:
            try:
                lock_loop = async_lock._loop
            except AttributeError:
                lock_loop = None
            if lock_loop is not None and lock_loop is not current_loop:
                # Stale lock – create a new one for this event loop
                if count == 0:
                    self._async_lock_cleanup_data.pop(combined_key, None)
                async_lock = asyncio.Lock()
                self._async_lock[combined_key] = async_lock
        ...
        pipeline_namespace.update(
            {
                ...
                "history_messages": history_messages,  # Use the shared list object
            }
        )
