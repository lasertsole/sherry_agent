...
def _load_config() -> Dict[str, Any]:
    try:
        import yaml
        cfg_path = ROOT_DIR / "config" / "curator.yaml"
        if cfg_path.exists():
...
def get_process_useless_skill() -> str:
    val = os.getenv("CURATOR_PROCESS_USELESS_SKILL", PROCESS_ARCHIVE).strip().lower()
    if val == PROCESS_DELETE:
