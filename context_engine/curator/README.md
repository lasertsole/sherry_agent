...
For each agent-created skill:
  │
  ├── pinned? → skip
  ├── cron-referenced? → skip  (removed, no cron-skill reference feature yet)
...
2. **Never auto-delete** — only archives. Archiving is recoverable.
...
├── helpers.py            # Utilities (ISO parsing, atomic writes)
