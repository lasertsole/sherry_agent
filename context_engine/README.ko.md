...
**마이그레이션:** 스키마 생성은 `_migrations` 테이블의 **이름 키** 방식으로 버전 관리됩니다(`name TEXT PRIMARY KEY, at INTEGER NOT NULL`). 각 단계는 안정된 이름을 가지며, `_migrate`는 목록 순서대로 "이름이 아직 기록되지 않은" 단계를 실행합니다——따라서 새 단계를 목록 어디에 삽입하든, 어떤 데이터베이스가 이미 지나친 위치보다 앞이어도 그 데이터베이스에서 반드시 실행됩니다(기존 인덱스 키 방식의 재개는 이를 조용히 건너뛰었고, 운영 데이터베이스에서 `reasoning_tokens` 컬럼이 유실된 원인입니다). 새 데이터베이스는 단일 완전 베이스라인(`build_schema_v1`, `0001_initial_schema`로 기록)을 실행해 위의 전체 스키마를 한 번에 생성합니다. 기존 인덱스 키 방식 추적 테이블(`v` 컬럼 보유)은 첫 연결 시 교체되며 이름이 전혀 미리 채워지지 않으므로, 멱등 베이스라인이 안전하게 재실행되고 스키마와 데이터는 그대로 유지됩니다. 단계 순서는 의존성 때문에 여전히 중요하므로 새 단계는 끝에 추가하는 것을 권장합니다——실행 여부를 결정하는 것은 위치가 아니라 이름입니다.
