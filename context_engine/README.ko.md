...
**마이그레이션:** 스키마 생성은 `_migrations` 테이블로 버전 관리됩니다. 새 데이터베이스는 단일 완전 베이스라인(`build_schema_v1`, v1로 기록)을 한 번 실행해 위의 전체 스키마를 생성합니다. 기존 증분 체인이 더 높은 워터마크에 남겨둔 데이터베이스는 베이스라인으로 정규화됩니다——`_migrate`는 경고를 남기고 워터마크를 현재 단계 수로 다시 씁니다(스키마와 데이터는 그대로 유지하고 베이스라인을 재실행하지 않음)——그래야 이후에 추가되는 단계가 계속 적용됩니다. 향후 스키마 변경은 v2, v3, … 로 끝에 추가하며 중간에 삽입하지 않습니다.
