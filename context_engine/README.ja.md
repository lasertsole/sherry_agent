...
**マイグレーション：** スキーマ作成は `_migrations` テーブルによる**名前キー**のバージョン管理です（`name TEXT PRIMARY KEY, at INTEGER NOT NULL`）。各ステップは安定した名前を持ち、`_migrate` はリスト順に「名前が未記録」のステップを実行します——したがって新しいステップをリストのどこに挿入しても、あるデータベースがすでに通過した位置より前であっても、そのデータベース上で必ず実行されます（旧インデックスキー方式のレジュームはこれを黙ってスキップし、本番データベースでは `reasoning_tokens` 列が失われました）。新規データベースは単一の完全なベースライン（`build_schema_v1`、`0001_initial_schema` として記録）を実行し、上記の完全なスキーマを一度に作成します。旧インデックスキー方式の追跡テーブル（`v` 列を持つ）は初回接続時に置き換えられ、名前は一切プリセットされないため、冪等なベースラインが安全に再実行され、スキーマとデータはそのまま保たれます。ステップの順序は依存関係上依然重要なので、新しいステップは末尾への追加を推奨します——実行可否を決めるのは位置ではなく名前です。
...
- **ライフサイクル**：`active → stale`（`stale_after_days`、デフォルト 30 日間無活動）。`archive_after_days`（デフォルト 90 日）を超えたスキルは `skills/.archive/` へアーカイブされ `archived` に（`curator restore <name>` で復元可能）。stale ウィンドウ内で一度も使われていないスキルは再アクティブ化されます。pinned スキルはすべての遷移をバイパスします。統合元（`ABSORBED_INTO` マーカー付き）と剪定スキルも同じアーカイブを使い、Curator の除去にハード削除はありません
...
タイトルクエリはユーザー送信元の `human` 行のみを受け付けます（旧行は `origin IS NULL`、または `origin = 'user'`）。`human` 行がすべて内部送信元（完了キャリア / TaskIntent / cron）であるセッションのタイトルは空文字列になり、クライアントがプレースホルダーを表示します。
