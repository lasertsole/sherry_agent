...
**マイグレーション：** スキーマ作成は `_migrations` テーブルでバージョン管理されています。新規データベースは単一の完全なベースライン（`build_schema_v1`、v1 として記録）を一度だけ実行し、上記の完全なスキーマを作成します。旧インクリメンタルチェーンによってより高いウォーターマークに置き去りにされたデータベースは、ベースラインへと正規化されます——`_migrate` は警告を発し、ウォーターマークを現在のステップ数に書き換えます（スキーマとデータはそのまま保持し、ベースラインを再実行しません）——これにより、今後追加されるステップが有効になります。将来のスキーマ変更は v2、v3、… として末尾に追加し、途中に挿入しません。
