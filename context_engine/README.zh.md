...
- **生命周期**：`active → stale`（`stale_after_days`，默认 30 天无活动）；超过 `archive_after_days`（默认 90 天）的技能会归档到 `skills/.archive/` 并标记为 `archived`（可用 `curator restore <name>` 恢复）；处于 stale 窗口内但从未使用过的技能会被重新激活。被 pinned 的技能跳过所有流转。合并源（带 `ABSORBED_INTO` 标记）与被剪枝技能走同一归档——Curator 的任何移除都不是硬删除
