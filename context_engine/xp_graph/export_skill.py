...
def export_all_communities() -> list[Path]:
    ...
    .. important::
       ...
       files only contain **name + description** summaries — agents that need
    ...
    """
    # ── First: ensure every skill has a standalone copy in single/ ──
    export_all_skills()
    ...
    # ── Group: community_id != None → named community  ──────────────
    #            community_id == None → orphan (individual)           ─
    ...
    # ── Export grouped communities (full detail for each member) ──
